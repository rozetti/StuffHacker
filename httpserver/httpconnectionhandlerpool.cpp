...
HttpConnectionHandlerPool::HttpConnectionHandlerPool(CRZSettings &settings, HttpRequestHandler* requestHandler)
    : QObject(),
      m_settings(settings)
{
    ...
    loadSslConfig();
    cleanupTimer.start(m_settings.value("cleanupInterval",1000).toInt());
...
HttpConnectionHandler* HttpConnectionHandlerPool::getConnectionHandler() {
    ...
    if (!freeHandler) {
        ...
        if (pool.count()<maxConnectionHandlers) {
            freeHandler=new HttpConnectionHandler(m_settings,requestHandler,sslConfiguration);
            freeHandler->setBusy();
...
//            #ifdef Q_OS_WIN32
//                if (QDir::isRelativePath(sslKeyFileName) && m_settings.format()!=QSettings::NativeFormat)
//            #else
...
//            }
//            #ifdef Q_OS_WIN32
//                if (QDir::isRelativePath(sslCertFileName) && m_settings.format()!=QSettings::NativeFormat)
//            #else
