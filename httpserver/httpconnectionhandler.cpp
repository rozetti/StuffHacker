...
HttpConnectionHandler::HttpConnectionHandler(CRZSettings &settings, HttpRequestHandler* requestHandler, QSslConfiguration* sslConfiguration)
    : QThread(),
      m_settings(settings)
{
    //Q_ASSERT(settings!=0);
...
void HttpConnectionHandler::read() {
    ...
    while (socket->bytesAvailable()) {
        ...
        if (!currentRequest) {
            currentRequest=new HttpRequest(m_settings);
        }
